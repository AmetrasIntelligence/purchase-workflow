...
class Common(SavepointCase):
    ...
    def setUpClassPackagingType(cls):
        cls.packaging_type_tu = cls.env["product.packaging.level"].create(
            {"name": "Transport Unit", "code": "TU", "sequence": 1}
        )
        cls.packaging_type_pl = cls.env["product.packaging.level"].create(
        ...
        cls.packaging_tu = cls.env["product.packaging"].create(
            {
                ...
                "packaging_level_id": cls.packaging_type_tu.id,
                "qty": TU_PRODUCT_QTY,
            }
        )
        cls.packaging_pl = cls.env["product.packaging"].create(
            {
                "name": "PACKAGING PL",
                "product_id": cls.product.id,
                "packaging_level_id": cls.packaging_type_pl.id,
                "qty": PL_PRODUCT_QTY,
            }
        )
        cls.packaging_cannot_be_purchased = cls.env["product.packaging"].create(
            {
                ...
                "product_id": cls.product.id,
                "qty": 10.0,
                "packaging_level_id": cls.packaging_type_cannot_be_purchased.id,
            }
        )
